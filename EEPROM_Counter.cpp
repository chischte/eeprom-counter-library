...
EEPROM_Counter::EEPROM_Counter(const int eepromSize, byte eepromNumberOfValues)
{
  ...
  int storeLocation;
  eeprom_read_block((void*) &storeLocation, (void*) 0, sizeof(2));
...
void EEPROM_Counter::update(byte valueNumber, long newValue)
{
  ...
  int destinationAddress = _storeLocation + 4 + valueNumber * 4; //shift 4 bytes for the writecounter and 4 bytes for every value
...
{
  //REPLACE EEPROM READ WITH ARRAY READ (faster)!!!!!


  int sourceAddress = _storeLocation + 4 + valueNumber * 4;
